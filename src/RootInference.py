...
transform = T.Compose([T.ToTensor(), T.Normalize((0.5, 0.5, 0.5), (0.1, 0.1, 0.1))])
...
with torch.no_grad():
    for i in range(1000, N):  # range(int(N/2), N):
        ...
        if bViewProgress:
            ...
            img[np.logical_and(mask_show < 0.99, mask_show > 0.9), :] = [
                0,
                102,
                0,
            ]  # Verde scuro, radice oltre 90% ma sotto 99%
            img[np.logical_and(mask_show < 0.9, mask_show > 0.8), :] = [
                0,
                255,
                255,
            ]  # Giallo, radice tra 90% e 90%
            img[np.logical_and(mask_show < 0.8, mask_show > 0.75), :] = [
                0,
                128,
                255,
            ]  # Arancio
            img[
                np.logical_and(mask_show < 0.3, mask_show > 0), 0
            ] = 128  # Radice entro il 30%
            cv2.imshow("Image", img)
